...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const multipliedVectors = x1 * x2 + y1 * y2;

  const vectorA = Math.sqrt(x1 * x1 + y1 * y1);
  const vectorB = Math.sqrt(x2 * x2 + y2 * y2);

  const angle = Math.acos(multipliedVectors / (vectorA * vectorB));
...
function toNumber(value, def) {
  const returnedNumber = Number(value);
  return Number.isNaN(returnedNumber) ? def : returnedNumber;
}
